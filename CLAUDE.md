...
다음 라운드(아직 브레인스토밍 전, 코딩 시작하지 말 것), 우선순위 순:
1. **ExpandedView UI 다듬기** — 배경/버튼/노브 색·스타일을 MiniView 수준으로
   맞추기(2026-08-23 `WA_StyledBackground` 고치면서 처음 실제로 렌더링된
   색이 사용자 눈에 이상해 보인다고 확인됨, `ExpandedView`는 버튼/노브
   QSS가 아예 없음), 리사이즈 시 버튼/노브도 패드/건반처럼 비율 스케일링,
   건반에 흑건/백건 구분 추가(지금은 15개 다 똑같은 박스).
2. **실제 MPK mini MK2 하드웨어 신호 연동** — 이번 세션까지는
   `python -m mpk_deck` 소프트웨어 단독 상호작용만 검증됨, 실제 컨트롤러의
   패드/노브/조이스틱/건반 MIDI 입력이 UI/액션에 실제로 연결되는지 end-to-end
   확인 필요.
3. ExpandedView 조이스틱을 마우스 드래그로 상하좌우 스크롤 액션에 매핑 +
   물리 MPK의 실제 조이스틱/피치벤드 MIDI 연결, 사용자가 지정한 건반 N개로
